...
void Sensor::init_rotary_encoder() {
  ...
  pcnt_config.pos_mode = !strcmp(type, "right") ? PCNT_COUNT_INC : PCNT_COUNT_DEC;//信号ピンに立ち上がりパルス(LOW->HIGH)が入った時どうするかの選択
  pcnt_config.neg_mode = !strcmp(type, "right") ? PCNT_COUNT_DEC : PCNT_COUNT_INC;//信号ピンに立ち下がりパルス(HIGH->LOW)が入った時どうするかの選択
