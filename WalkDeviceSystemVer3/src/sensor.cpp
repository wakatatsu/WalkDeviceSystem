...
void Sensor::init_rotary_encoder() {
  for(int i = 0; i < ROTARY_ENCODER_NUM; i++) {
    ...
    pcnt_config[i].pos_mode = (i==0) ? PCNT_COUNT_DEC : PCNT_COUNT_INC;//信号ピンに立ち上がりパルス(LOW->HIGH)が入った時どうするかの選択
    pcnt_config[i].neg_mode = (i==0) ? PCNT_COUNT_INC : PCNT_COUNT_DEC;//信号ピンに立ち下がりパルス(HIGH->LOW)が入った時どうするかの選択
